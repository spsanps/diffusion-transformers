...
BartConfig.encoder_guassian_noise = 0.0
BartConfig.encoder_gaussian_ratio_low = None
...
class BartEncoder(BartPreTrainedModel):
    ...
    def forward(
        ...
        # add guassian noise to hidden states based on config
        if self.config.encoder_gaussian_ratio:
            noise = torch.normal(mean=0, std=1, size=hidden_states.shape).to(
                hidden_states.device
            )
            noise_r = self.config.encoder_gaussian_ratio
            if self.config.encoder_gaussian_ratio_low:
                low_noise_r = self.config.encoder_gaussian_ratio_low
                # randomly choose a ratio between low and high noise ratio range
                r = torch.rand(1).item() * (noise_r - low_noise_r) + low_noise_r
            else:
                r = noise_r

            hidden_states = hidden_states * (1 - r) + noise * r
